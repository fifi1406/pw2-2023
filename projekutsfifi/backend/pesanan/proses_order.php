...
$_costumer_id = $_POST['costumer_id'];
...
if($_proses == "Simpan"){
    ...
    $sql = "INSERT INTO order (order_number,date,qty,total_price,costumer_id,product_id)VALUES (?,?,?,?,?,?)";
}else if($_proses == "Update"){
    // Jika Update, tambahkan ID ke array dan buat SQL UPDATE
    $ar_data[]=$_POST['id'];
    $sql = "UPDATE order SET order_number=?,date=?,qty=?,total_price=?,costumer_id=?,product_id=? WHERE id=?";
}
